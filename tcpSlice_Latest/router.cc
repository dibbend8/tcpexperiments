...
void Router::initialize()
{
    ...
    updateinterval = 5e-3; //every 500 ms
    scheduleAfter(updateinterval, updateprice);
    estrate = 0;totalrecv=0;
    pktSize = getAncestorPar("packetSize").intValue();//in bytes
...
void Router::handleMessage(cMessage *msg)
{
    ...
    if(strcmp(msg->getName(),"data")==0)
    {
        ...
        totalrecv++;
        recv[data->getKind()]++;
        //        if(reqQ.getLength()<buffcap)
        //        {
        //            reqQ.insert(data);
        //        }
        if(simTime() - data->getCreationTime()<= 1e-3)
        {
            reqQ.insert(data);
            ...
            if (txFinishTime <= simTime()) {
                ...
                if(getIndex()==2)
                {
                    ...
                    send(pkt->dup(), "toserver$o",data->getKind());
                    delete pkt;
                }
                else
                {
                    cPacket *pkt = reqQ.pop();
                    send(pkt->dup(), "toserver$o",0);
                    delete pkt;
                }
            }
            else {
    ...
    else if(msg==endTxMsg)
    {
        ...
        if (txFinishTime <= simTime()) {
            ...
            {
                cPacket *pkt = reqQ.pop();
                send(pkt->dup(), "toserver$o",pkt->getKind());
                delete pkt;
            }
            else
            {
    ...
    else if(msg==updateprice && getIndex()!=2)
    {
        ...
        scheduleAfter(updateinterval, updateprice);
        estrate = (double)(totalrecv*pktSize*8)/updateinterval;
        Sdm *sdnmsg = new Sdm("rates");
        sdnmsg->setRatesArraySize(numhosts);

        if(getIndex()==0)
        {
            sdnmsg->setRates(0, (double) (recv[0]*pktSize*8)/updateinterval);
            sdnmsg->setRates(1, (double) (recv[1]*pktSize*8)/updateinterval);
        }
        else
        {
            sdnmsg->setRates(2, (double) (recv[2]*pktSize*8)/updateinterval);
        }
        ...
        EV<<"Computing the difference in rates  = "<<(c-estrate)<<endl;
        EV<<"Increment Step  = "<<lp*(c-estrate)<<endl;
        price  =  fmax(price-lp*(c - estrate),0);
        //        price = price-lp*(c-estrate);
